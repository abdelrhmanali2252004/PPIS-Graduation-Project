...
function LogoGeometric() {
  return (
    <svg viewBox="0 0 120 80" className="h-full w-full">
      <polygon points="40,10 70,10 85,35 70,60 40,60 25,35" fill="#1B4C8C" opacity="0.9" />
  ...
  return (
    <div className="px-8 py-8 pb-20 font-cairo md:px-10" dir="rtl">
      <header className="mb-6">
        <h1 className="text-xl font-bold text-body md:text-2xl">الخطوة الرابعة: الهوية بالذكاء الاصطناعي</h1>
        <p className="mt-1 text-sm text-slateMuted">٣ خطوات فرعية لبناء علامتك</p>
      </header>
        ...
        {['الطابع البصري', 'لوحة الألوان', 'توليد الشعار'].map((label, i) => (
          <div key={label} className="flex items-center">
            <button
              ...
              className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold transition-all ${i === sub
            ...
            {VIBES.map((v) => {
              ...
              return (
                <button
                  ...
                  className={`relative rounded-2xl border-2 p-5 text-right transition-all ${sel ? 'border-nile bg-nile/5 shadow-md' : 'border-divider bg-white hover:border-nile/30'
                    }`}
                >
                  {sel && (
            ...
            {PALETTES.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full flex-col gap-3 rounded-2xl border-2 p-4 text-right transition-all ${sel ? 'border-gold bg-gold/5 shadow-md' : 'border-divider bg-white'
                    }`}
                >
        ...
        {sub === 2 && (
          <div className="space-y-6">
            <div className="animate-pulse-glow rounded-xl border border-gold/40 bg-nile/5 px-4 py-3">
              <div className="mb-1 text-xs font-bold text-nile">جاري توليد المفاهيم بالذكاء الاصطناعي</div>
            ...
            <div className='flex gap-1 flex-wrap'>
              ...
              {[
                ...
                <div key={id} className=" rounded-2xl border border-divider bg-white p-4 shadow-sm">
                  <div className="mb-2 font-bold text-nile">{title}</div>
                  <div
                    className={`mx-auto mb-3 overflow-hidden rounded-xl border border-divider bg-offwhite ${preview[id] === 'mobile' ? 'max-w-[120px]' : 'max-w-full'
                      }`}
                    style={{ aspectRatio: preview[id] === 'mobile' ? '9/16' : '3/2' }}
                  >
                    <Node />
                  </div>
                  <div className="mb-2 flex flex-wrap gap-2">
                    <button
                      ...
                      className={`rounded-lg px-3 py-1 text-xs font-semibold ${preview[id] === 'card' ? 'bg-nile text-white' : 'bg-offwhite'
                        }`}
                    >
                      بطاقة عمل
                    </button>
                    <button
                      ...
                      className={`rounded-lg px-3 py-1 text-xs font-semibold ${preview[id] === 'mobile' ? 'bg-nile text-white' : 'bg-offwhite'
                        }`}
                    >
                      شاشة موبايل
                    </button>
            ...
            <div className="rounded-xl border border-divider">
              ...
              {accordionOpen && (
                <div className="space-y-3 border-t border-divider bg-offwhite/50 px-4 py-4">
                  ...
                  {sent && <p className="text-center text-xs font-semibold text-success">تم استلام طلبك ✓</p>}
                </div>
              )}
            </div>
          </div>
