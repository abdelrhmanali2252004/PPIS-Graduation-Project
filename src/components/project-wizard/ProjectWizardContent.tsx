...
import { QUESTION_ITEMS, type ProjectAnswerKey, type ProjectAnswers } from './questions'
...
export default function ProjectWizardContent({
 ...
 const navigate = useNavigate();
  ...
  return (
    <div className="flex min-h-0 flex-col items-center overflow-y-auto px-6 py-8 pb-16 font-cairo" dir="rtl">
      <div className="w-full max-w-[680px]">
        <header className="mb-6 text-center">
          <h1 className="text-xl font-bold text-body">الخطوة الثانية: تفاصيل المشروع</h1>
            ...
            {Array.from({ length: 12 }, (_, i) => {
              ...
              return (
                <div key={n} className="flex flex-1 items-center">
                  ...
                  {i < 11 && (
                    <div className={`mx-0.5 h-0.5 flex-1 ${done ? 'bg-gold' : 'bg-divider'}`} />
                  )}
                </div>
              )
            })}
          </div>
          <p className="mt-3 text-center text-xs text-slateMuted">السؤال {step} من ١٢</p>
        </div>
